"""
Model-agnostic Phase-1 plotting. Generalizes figures_llama.py: point it at any
model's results dir (GPT-2 / Llama / OPT / Qwen) and it auto-detects the layer
count from the CSV columns.

Produces, under figures/<results-dir basename>/:
  - behavioral.png       (log perplexity, output divergence, logit KL)
  - representation.png   (stripped cosine "content" vs stripped CKA "geometry")
  - metric_heatmaps.png  (raw cosine / stripped cosine / stripped CKA, layer x %)
  - in_vs_out.png        (optional, --with-input-div; needs tokenizer)

Usage:
  python figures_model.py --results-dir /ssd_scratch/varghese/decoding_robustness/results/opt_6.7b --name "OPT-6.7B"
  python figures_model.py --results-dir .../results/qwen2.5_7b --name "Qwen2.5-7B"
"""
...
ap.add_argument("--results-dir", required=True,
                help="dir containing char/ token/ shuffle/ <pct>/evals.csv")
ap.add_argument("--name", required=True, help="display label, e.g. 'OPT-6.7B'")
ap.add_argument("--out-dir", default=None,
                help="output dir (default: figures/<results-dir basename>)")
ap.add_argument("--with-input-div", action="store_true",
                help="also plot input-vs-output divergence (needs --model + its tokenizer)")
ap.add_argument("--model", default=None,
                help="HF model id, only needed for --with-input-div")
args = ap.parse_args()

RESULTS = args.results_dir.rstrip("/")
NAME = args.name
OUTDIR = args.out_dir or f"figures/{os.path.basename(RESULTS)}"
...
# auto-detect layer count from a populated CSV
sample = load("char", REP_PCT["char"])
nL = sum(c.startswith("activation_cka_layer_") for c in sample.columns)
layers = list(range(nL))
print(f"{NAME}: detected {nL} layers (hidden states)")

# ---------- Figure 1: behavioral ----------
fig, axs = plt.subplots(1, 3, figsize=(18, 5))
fig.suptitle(f"{NAME} — Behavioral metrics", fontsize=14)
for t in PTB:
    ps = pcts(t)
...
fig.suptitle(f"{NAME} — Representation similarity to clean, per layer", fontsize=14)
...
fig.suptitle(f"{NAME} — Representation similarity to clean (layer x perturbation %)", fontsize=15)
im = None
for c, t in enumerate(PTB):
    ...
    for r, (mname, prefix) in enumerate(METRICS):
        ...
        xt = list(range(0, nL, max(1, nL // 8))); ax.set_xticks(xt); ax.set_xticklabels([f"L{L}" for L in xt], fontsize=7)
        if r == 0:
...
if args.with_input_div:
    assert args.model, "--with-input-div needs --model <hf id>"
    import random
    import Levenshtein
    ...
    fig.suptitle(f"{NAME} — Input vs Output divergence", fontsize=14)
    for i, t in enumerate(PTB):
        ps = pcts(t)
        indiv = []
        for p in ps:
