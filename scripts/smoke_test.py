...
def probe(model_key, batch_size=2, seq_len=128):
    ...
    ids = torch.randint(0, min(tokenizer.vocab_size, 50000),
...
def main():
    ...
    print(f'{"model":<16}{"dtype":<6}{"weights":>8}{"load_s":>8}{"fwd_s":>8}{"peak_GB":>9}{"VRAM%":>7}')
    total = torch.cuda.get_device_properties(0).total_memory / 1e9
    for key in args.models.split(','):
        key = key.strip()
        if key not in MODEL_INFO:
            print(f'{key:<16} NOT IN MODEL_INFO'); continue
        info = MODEL_INFO[key]
        try:
            load_s, fwd_s, peak = probe(key)
            print(f'{key:<16}{info["dtype"]:<6}{"~":>8}{load_s:>8.1f}{fwd_s:>8.2f}'
