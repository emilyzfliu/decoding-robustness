...
def canonical_column_order(df):
    ...
    n_attn = len(
        {int(ATTN_COL_RE.match(c).group(1)) for c in df.columns if ATTN_COL_RE.match(c)}
    )
    cols = list(BASE_COLS)
    ...
    for i in range(n_hidden):
        cols += [
            f"intrinsic_dim_clean_layer_{i}",
            f"intrinsic_dim_perturbed_layer_{i}",
            f"intrinsic_dim_change_layer_{i}",
        ]
    for i in range(n_hidden):
        cols += [
            f"intrinsic_dim_mknn_clean_layer_{i}",
            f"intrinsic_dim_mknn_perturbed_layer_{i}",
...
def convert_file(path, dry_run=False):
    ...
    for i in layers:
        ...
        layer_means[f"attn_entropy_layer_{i}"] = (
            df[head_cols].mean(axis=1).astype(np.float32)
        )
    ...
    for i in layers:
        df.insert(
            df.columns.get_loc("logit_kl"),
            f"attn_entropy_layer_{i}",
            layer_means[f"attn_entropy_layer_{i}"],
        )
    ...
    for c in df.columns:
        if "intrinsic_dim" in c:
            df[c] = (
                df[c].replace([np.inf, -np.inf], np.nan).fillna(0.0).astype(np.float32)
            )
    ...
    if dry_run:
        print(
            f"  would convert: {len(attn_cols)} attn cols -> {len(layers)} layer means, "
            f"~{old_size_mb:.0f} MB -> ~{old_size_mb * len(layers) / len(attn_cols):.1f} MB (est.)",
            flush=True,
        )
        return True
    df.to_csv(path, index=False, float_format="%.6f")
    print(
        f"  converted: {len(attn_cols)} attn cols -> {len(layers)} layer means, "
        f"{old_size_mb:.0f} MB -> {os.path.getsize(path) / 1e6:.1f} MB",
        flush=True,
    )
    return True
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert legacy per-head attention CSVs to lean schema"
    )
    parser.add_argument("--out-root", default="results_v2")
    parser.add_argument(
        "--models",
        default=None,
        help="Comma-separated model keys (default: all in out_root)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
