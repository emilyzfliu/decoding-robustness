...
def _read_attention_entropy(path, model):
    layer_cols = [
        f"attn_entropy_layer_{i}" for i in range(MODEL_INFO[model]["num_layers"])
    ]
    try:
        return pd.read_csv(path, usecols=layer_cols)
    except ValueError:
        head_cols = [
            f"attn_layer{i}_head_{h}_entropy_norm"
            for i in range(MODEL_INFO[model]["num_layers"])
            for h in range(MODEL_INFO[model]["num_heads"])
        ]
        legacy = pd.read_csv(path, usecols=head_cols)
        return pd.DataFrame(
            {
                f"attn_entropy_layer_{i}": legacy[
                    [
                        f"attn_layer{i}_head_{h}_entropy_norm"
                        for h in range(MODEL_INFO[model]["num_heads"])
                    ]
...
def compute_attention_entropy_metrics_comparison(ptb_type, n_bins=5):
    ...
    for model in MODEL_INFO.keys():
        ...
        ptb_pcts = (
            [x * 5 for x in range(11)]
            if ptb_type != "shuffle"
            else [x * 5 for x in range(21)]
        )

        dfs_by_pct = {}
        for pct in ptb_pcts:
            try:
                seq = _read_attention_entropy(
                    f"results/{model}/{ptb_type}/{pct}/evals.csv", model
                )
            except (FileNotFoundError, ValueError):
                seq = _read_attention_entropy(
                    f"results/{model}/char/0/evals.csv", model
                )
            dfs_by_pct[pct] = seq
        ...
        for i in range(num_layers):
            ...
            means = np.array(
                [dfs_by_pct[pct][f"attn_entropy_layer_{i}"].mean() for pct in ptb_pcts]
            )
            y_centered = means - means.mean()
            slopes = np.array([(x_centered * y_centered).sum() / denom])

            all_data["slope"].extend(slopes.tolist())
            all_data["depth_bin"].append(b)
            all_data["model"].append(MODEL_PLOT_CONFIG[model]["name"])
    ...
    sns.violinplot(
        ...
        hue="model",
        palette={
            MODEL_PLOT_CONFIG[m]["name"]: MODEL_PLOT_CONFIG[m]["color"]
    ...
    ax.set_title(
        f"Attention Entropy Change vs Perturbation, by Relative Depth ({ptb_type} perturbation)"
    )
...
def compute_attention_entropy_metrics(ptb_type):
    for model in MODEL_INFO.keys():
        ...
        for i in layer_indices:
            ...
            plot_dat["slope"].append(slope)
            plot_dat["layer"].append(i)
        ...
        sns.violinplot(
            ...
            x="layer",
            y="slope",
            palette=[MODEL_PLOT_CONFIG[model]["color"]],
            hue="layer",
            legend=False,
            ax=ax,
        )
        ax.set_title(
            f"{MODEL_PLOT_CONFIG[model]['name']} Attention Entropy Change vs Perturbation Percentage ({ptb_type} perturbation)"
        )
...
def compute_aggregate_output_metric(metric_name, ptb_type):
    ...
    for model in MODEL_INFO.keys():
        plt.plot(
            df["ptb_pct"],
            df[model],
            label=MODEL_PLOT_CONFIG[model]["name"],
            color=MODEL_PLOT_CONFIG[model]["color"],
        )
    plt.xlabel("Perturbation Percentage")
    plt.ylabel(metric_name)
    plt.title(
        f"Aggregate {metric_name} vs Perturbation Percentage for {ptb_type} Perturbation"
    )
    plt.legend()
