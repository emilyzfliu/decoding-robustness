                ...
                if metric_cols:
                    if layer_agg == "mean":
                        ...
                        vals = [
                            ...
                            if any(f"_{l}" in col for l in layer_agg)
                        ]
...
fig.suptitle(
    "BPE vs Word-Level Substitution: Representation Metrics",
    fontsize=14,
    fontweight="bold",
)
...
for idx, pct in enumerate([5, 10, 25, 50]):
    ...
    ax.bar(
        x - width / 2, cka_bpe, width, label="BPE Token", color="#2166ac", alpha=0.85
    )
    ax.bar(
        x + width / 2, cka_word, width, label="Word-Level", color="#d6604d", alpha=0.85
    )
...
if has_twonn:
    ...
    fig, axs = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(
        "Per-Layer TwoNN Intrinsic Dimension Change", fontsize=14, fontweight="bold"
    )
    for idx, pct in enumerate([5, 10, 25, 50]):
        row, col = idx // 2, idx % 2
        ax = axs[row, col]
        ...
        df_word = pd.read_csv(f"results/compare_bpe_word/word/{pct}/evals.csv")
        twonn_bpe = [
            df_bpe[f"intrinsic_dim_change_layer_{i}"].mean() for i in range(12)
        ]
        twonn_word = [
            df_word[f"intrinsic_dim_change_layer_{i}"].mean() for i in range(12)
        ]
        x = np.arange(12)
        width = 0.35
        ax.bar(
        ...
        ax.set_xlabel("Layer")
        ax.set_ylabel("TwoNN Dim Change")
        ax.set_title(f"{pct}%")
        ax.set_xticks(x)
        ax.set_xticklabels([f"L{i}" for i in range(12)])
        ax.legend()
...
if has_mknn:
    ...
    fig, axs = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(
        "Per-Layer MKNN Intrinsic Dimension Change", fontsize=14, fontweight="bold"
    )
    for idx, pct in enumerate([5, 10, 25, 50]):
        row, col = idx // 2, idx % 2
        ax = axs[row, col]
        ...
        df_word = pd.read_csv(f"results/compare_bpe_word/word/{pct}/evals.csv")
        mknn_bpe = [
            df_bpe[f"intrinsic_dim_mknn_change_layer_{i}"].mean() for i in range(12)
        ]
        mknn_word = [
            df_word[f"intrinsic_dim_mknn_change_layer_{i}"].mean() for i in range(12)
        ]
        x = np.arange(12)
        width = 0.35
        ax.bar(
...
if n_panels == 1:
    axs = [axs]
fig.suptitle(
    "All Metrics at 25% Perturbation (Per Layer)", fontsize=14, fontweight="bold"
)
...
cka_word = [df_word[f"{cka_prefix}layer_{i}"].mean() for i in range(12)]
axs[panel_idx].plot(
    x, cka_bpe, "o-", color="#2166ac", label="BPE Token", linewidth=2, markersize=6
)
axs[panel_idx].plot(
    x, cka_word, "s-", color="#d6604d", label="Word-Level", linewidth=2, markersize=6
)
...
if has_mknn:
    ...
    axs[panel_idx].plot(
        x, mknn_bpe, "o-", color="#2166ac", label="BPE Token", linewidth=2, markersize=6
    )
    axs[panel_idx].plot(
        x,
        mknn_word,
