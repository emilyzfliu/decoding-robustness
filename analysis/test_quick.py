...
def test_metrics():
    ...
    inputs = tokenizer(
        texts, return_tensors="pt", truncation=True, max_length=32, padding="max_length"
    )
    ...
    with torch.no_grad():
        ...
        outputs_ptb = model(
            **inputs_ptb, output_hidden_states=True, output_attentions=True
        )
    ...
    try:
        ...
        cka_keys = [
            k for k in cka_result.keys() if k.startswith("activation_cka_layer_")
        ]
