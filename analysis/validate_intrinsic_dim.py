...
def main():
    ...
    print(
        f"Collecting hidden states: {args.model}, {args.n_seq} seqs, "
    ...
    fig.suptitle(
        f"Intrinsic-dim estimator convergence ({args.model}, {args.ptb_type}@{args.ptb_pct}%)",
        fontsize=13,
    )
    ...
    for L in LAYERS:
        for name, est in estimators.items():
            for setting, pts in [("clean", pts_clean[L]), ("perturbed", pts_ptb[L])]:
                ...
                print(
                    f"  L{L:>2d} {name:>4s} {setting:<10} {m:7.2f} [{lo:7.2f}, {hi:7.2f}]"
                )
    ...
    print(
        "  - Narrow CI relative to the gap clean-vs-perturbed => the effect is statistically reliable."
    )
