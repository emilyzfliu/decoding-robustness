...
def id_ablation_heads_entropy(args):
    ...
    for i in range(12):
        for h in range(12):
            ...
            scores[(i, h, ptb_pct[-1], "diffuse" if slopes[i, h] > 0 else "sink")] = (
                score
            )
...
def run_ablation(args, ablate_type, ptb_pct, l, h):
    ...
    os.makedirs(
        f"results/{args.model}/ablated/{ptb_type}/l={l}_h={h}_pct={ptb_pct}_{ablate_type}",
        exist_ok=True,
    )
    ...
    try:
        seen = set(
            pd.read_csv(
                f"results/{args.model}/ablated/{ptb_type}/l={l}_h={h}_pct={ptb_pct}_{ablate_type}/evals.csv"
            )["sample"]
        )
    except:
        seen = set()
    ...
    for i in tqdm(range(0, len(texts), BATCH_SIZE), desc=f"{ptb_type} pct={ptb_pct}"):
        ...
        with torch.no_grad():
            ...
            outputs_perturbed = model(
                **inputs_perturbed, output_hidden_states=True, output_attentions=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        help="Model name (must be in config.MODEL_INFO)",
        type=str,
        default="gpt2",
    )
    parser.add_argument(
