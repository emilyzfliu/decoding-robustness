...
def get_cka_prefix(df):
    ...
    for c in df.columns:
        ...
        if c.startswith("cka_layer_"):
            return "cka_"
    raise ValueError(
        f"No CKA columns found in evals.csv (columns: {list(df.columns)[:10]}...)"
    )
...
def load_type_data(model, ptb_type, pct):
    ...
    if not os.path.exists(path):
        return None
    df = pd.read_csv(
        path, usecols=lambda c: c.startswith("activation_cka_") or c.startswith("cka_")
    )
...
def load_per_sample_cka(model, ptb_type, pct):
    ...
    prefix = get_cka_prefix(df)
    cols = sorted(
        [c for c in df.columns if c.startswith(prefix + "layer_")],
...
def run_stats(model):
    ...
    for pct in [5, 25]:
        ...
        for a, b in pairs:
            ...
            print(
                f"  {a:>8} ({samples[a].mean():.3f}) vs {b:<8} "
...
def main():
    ...
    parser.add_argument(
        "--model", default="gpt2", help="Model key in config.MODEL_INFO"
    )
    parser.add_argument(
        "--pct",
        type=int,
        default=25,
        help="Percentage for the per-layer/heatmap figures",
    )
    ...
    fig.suptitle(
        f"Per-Layer Stripped CKA by Perturbation Type ({args.model}, {pct}%)",
        fontsize=13,
    )
