...
parser = argparse.ArgumentParser(
    description="Generate figures from base experiment results."
)
parser.add_argument(
    "--model", default="gpt2", help="Model key in config.MODEL_INFO (default: gpt2)"
)
parser.add_argument(
    "--out-dir", default="figures", help="Where to write figure/CSV outputs"
)
...
percentages = [
    [x * 5 for x in range(1, 11)] if x != "shuffle" else [x * 5 for x in range(1, 21)]
    for x in ptb_types
]
...
def plot_ablation_figure(ptb_types, ablation_dfs, lbl):
    ...
    for idx, ptb in enumerate(ptb_types):
        ...
        markers = [
            ENTROPY_MARKERS.get(df.iloc[i]["true_class"], "o") for i in range(len(df))
        ]
        ...
        ax.axhline(
            -threshold,
            color=CLASS_COLORS["compensator"],
            linewidth=0.8,
            linestyle="--",
            alpha=0.7,
        )
        ...
        ax.set_xticks(xs)
        ax.set_xticklabels(
            [f"L{x}H{y}" for x, y in heads], rotation=45, ha="right", fontsize=8
        )
    ...
    handles = [
        Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=CLASS_COLORS["propagator"],
            markersize=8,
            label="Propagator",
        ),
        Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=CLASS_COLORS["compensator"],
            markersize=8,
            label="Compensator",
        ),
        Line2D(
            ...
            marker="o",
            color="w",
            markerfacecolor=CLASS_COLORS["neutral"],
            markersize=8,
            label="Neutral",
        ),
        Line2D(
...
fig.suptitle(
    "Per-Layer Representation Similarity: raw cosine (confounded) vs stripped CKA",
    fontsize=14,
)
...
for i, PTB_TYPE in enumerate(ptb_types):
    ...
    axs[i].plot(
        layers, raw_cos, "o--", color="#999999", label="raw cosine (confounded)"
    )
...
plt.savefig(
    f"{OUT_DIR}/representation_similarity_corrected.png", bbox_inches="tight", dpi=150
)
...
for i, PTB_TYPE in enumerate(ptb_types):
    ...
    for pct in percentages[i]:
        seq = pd.read_csv(f"results/{MODEL}/{PTB_TYPE}/{pct}/evals.csv")
        mean_matrix.append(
            [np.mean(seq[f"activation_cka_layer_{L}"]) for L in range(13)]
        )
    mean_matrix = np.array(mean_matrix)
    ...
    sns.heatmap(
        ...
        vmin=0,
        vmax=1,
        yticklabels=percentages[i],
        xticklabels=[f"L{L}" for L in range(13)],
    )
    axs[i].set_title(ptb_names[i])
    axs[i].set_xlabel("Layer")
