"""
Model-agnostic k-sensitivity diagnostic for the representation metric.
(Generalizes llama_k_diagnostic.py — works for GPT-2 / OPT / Qwen / Llama.)
...
the right k for this model. k=5 was tuned on GPT-2 (768-dim); wide models
(OPT/Qwen/Llama are 3.5k-4k dim) generally need a larger k, so sweep further.

Runs at ONE representative perturbation level (the k-at-convergence is a property
of the model's activation geometry, not the corruption level), on --n passages.

Run with the same env as the sweep (HF_HOME + HF login for gated models):
  python k_diagnostic.py --model facebook/opt-6.7b --ptb-type char --ptb-pct 25 --n 40
  python k_diagnostic.py --model Qwen/Qwen2.5-7B   --ptb-type char --ptb-pct 25 --n 40
  # wider sweep for a 4k-dim model:
  python k_diagnostic.py --model facebook/opt-6.7b --ks 0,10,25,50,100,250,500,1000
"""
...
def main():
    ...
    ap.add_argument("--ks", default="0,5,10,20,50,100,200,500",
    ...
    print(f"\nmodel={args.model}  d_model={d_model}  {args.ptb_type}@{args.ptb_pct}%  n={nseen}  layers={nL}")
    print(f"k sweep: {KS}   (k as % of d_model: {[f'{100*k/d_model:.1f}%' for k in KS]})")
